...
<!-- <script src='<?php echo TEMPLATE_DIR; ?>assets/js/script.js'></script> -->
...
<form class="search-bar flex flex-grow lg:w-auto items-center" method="post" id="searchParam">
    ...
    <div class="keyboard-container-search-bar flex-grow">
        <input type="text" name="q" id="q" class="w-full flex-grow text-sm h-9  place keyboard q border-tolidatColor bg-gray-50 border-2 focus:outline-none rounded rounded-l-none px-3" placeholder="جست و جو در بین کسب و کارها ..." value="<?php echo isset($list['q']) ? $list['q'] : '' ?>">
    </div>
    <!-- <select name="type" id="type" class="hidden">
            <option value="تولیدی" < ?= (isset($list['type']) && $list['type'] == 'تولیدی') ? 'selected' : '' ?>>تولیدی</option>
        </select> -->

    <input type="hidden" name="type" id="type" value="تولیدی">
