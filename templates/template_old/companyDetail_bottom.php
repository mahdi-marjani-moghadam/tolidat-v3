...
<!-- modal-img-->
<div class="holder-modal modal fade" id="myModal-img" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
...
<script>
    $(function () {
        ...
        var url = window.location.href.split('/').filter(function(v){return v!==''})[2];

        $('.tab .tabMenu li').each(function() {
            var menuUrl = $(this).find('a').attr('href');

            if(menuUrl !== undefined) {
                var tmp = menuUrl.split('/').filter(function(v){return v!==''})[2];
