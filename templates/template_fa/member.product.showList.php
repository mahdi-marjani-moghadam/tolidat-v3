...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12">
        <div class="holder-title center-block">
            ...
            <button data-position="right" data-intro="اضافه کردن محصول" id="addForm" type="button" <?php //echo($list['product_count'] >= $list['all_product_count'] ? 'disabled' : '') ?> class="btn btn-sm pull-left add-btnPro">
...
<div class="row add-product">
    <?php if (isset($list['list']) && count($list['list'])): ?>
        <?php foreach ($list['list'] as $id => $fields): ?>
            <div class="col-xs-12 col-sm-6 col-md-4 pull-right mb5 remove-product" data-value="<?= $fields['Product_d_id'] ?>">
                <div data-intro="لیست محصولات" class="contentPro<?php echo ($fields['status'] == 1) ? '' : ' disable' ?> whiteBg roundCorner boxBorder" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="">
                    <h3>
                        <div class="kebabMenu">
                            ...
                            <ul class="kebab-menu-content roundCorner boxBorder">
                                <li>
                                    <a class="link-edit" data-value="<?= $fields['Product_d_id'] ?>"><i class="fa fa-pencil" aria-hidden="true"></i><span>ویرایش </span></a>
                                </li>
                                <li>
                                    <a class="link-trash" data-value="<?= $fields['Product_d_id'] ?>"><i class="fa fa-trash-o" aria-hidden="true"></i><span>حذف</span></a>
                                </li>
                                <li>
                                    <a  class="link-addPicture" data-value="<?= $fields['Product_d_id'] ?>"><i class="fa fa-plus-circle" aria-hidden="true"></i><span> گالری محصول</span></a>
                                </li>
                            </ul>
                        </div>
                        <div class="logo">
                    ...
                    <ul class="content-description">
                        <?php foreach ($fields['gallery'] as $gallery) : ?>
                            <li data-value="<?= $gallery['product_gallery_id'] ?>">
                    ...
                    <div class="row noMargin">
                        <div class="col-xs-12 col-sm-12 col-md-6 pull-right">
                            ...
                            <div class="form-group mb">
                                ...
                                <input name="meta_keyword" type="text"
                                       data-error="لطفا کلمه کلیدی را وارد نمایید"
                                       placeholder="کلمات کلیدی مرتبط را وارد نمایید. (بعد از هر کلمه دکمه Enter را بزنید.)"
                                       data-role="tagsinput">
                            </div>
                        </div>
...
<script>
    $(function () {
        ...
        $body.on('click', '.link-addPicture', function (e) {
            ...
            $.post('/member/product/showGallery/', {product_d_id: $(this).data('value')}, function (data) {
            ...
            iziToast.question({
                ...
                buttons: [
                    ['<button class="btn btn-success btn-sm pull-right" style="margin-left: 1em;">بله</button>', function (instance, toast) {
                        ...
                        $.post('/member/product/deleteGallery/', {product_gallery_id : productGalleryId}, function (data) {
                            var response = $.parseJSON(data);
                            if (response.result == -1) {
                                $.iziToastError(response.msg, '.iziToastAdd-container');
                                   return;
        ...
        function editItem(dataID, $this) {
            ...
            $.post('/member/product/editAjax/', {id: dataID}, function (data) {
