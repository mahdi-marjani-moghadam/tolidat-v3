 ...
 if(in_array($extension, $allowed_extension)) //check selected file extension is present in allowed extension array
 {
  ...
  foreach ($objPHPExcel->getWorksheetIterator() as $worksheet)
  {
   ...
   for($row=1; $row<=$highestRow; $row++)
   {
    ...
    $id++;
    $label = mysqli_real_escape_string($connect, $worksheet->getCellByColumnAndRow(1, $row)->getValue());
    ...
    $query = "INSERT INTO m_menu(id,parent_id,component,action,label,module_name,sort)
               VALUES
               ('".$id."','".$parent_id."','".$component."','".$action."'
               ,'".$label."','".$module_name."','".$id."')";
