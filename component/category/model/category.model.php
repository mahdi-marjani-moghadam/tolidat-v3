...
class categoryModel
{
    ...
    public function getCategoryUlLiSearch($CategoryTree, $parent_id = 0)
    {
        ...
        $result['result'] = 1;
        $result['list'] = $mainMenu;
        return $result;
    }

    public function getCategoryUlLiMember($CategoryTree, $parent_id = 0)
    {
        $mainMenu = $this->convertTreetoLiUlMember($CategoryTree[$parent_id], 1, $CategoryTree);
        //$mainMenu = "<ul>\n".$mainMenu ."</ul>";
        $result['result'] = 1;
        $result['export']['list'] = $mainMenu;
    ...
    public function convertTreetoLiUl($array = array(), $root = 0, $all = array())
    {
        static $mainMenu = '';
        static $mainList;
        if ($root == 1) {
        ...
        foreach ($array as $key => $val) {
            ...
            if (is_array($mainList[$val['Category_id']])) {
                $mainMenu .= '<li><a href=" ' . RELA_DIR . $cityStr . 'company/type/تولیدی/category/' . $val['Category_id'] . '"' . ' >' . $val['title'] . '</a>';
                $this->convertTreetoLiUl($mainList[$val['Category_id']]);
                $mainMenu .= '</li>';
            } else {
                $mainMenu .= "\t" . '
                    <li>
                        <a href=" ' . RELA_DIR . $cityStr . 'company/type/تولیدی/category/' . $val['Category_id'] . '"' . ' >' . $val['title'] . '</a>
                    </li>
                ' . "\n";
            }
            //$mainMenu .= "</ul>\n";
    ...
    public function convertTreeToLiUlMember($array, $root = 0, $all = array())
    {
        ...
        foreach ($array as $key => $val) {
            ...
            if (is_array(self::$mainList[$val['Category_id']])) {
                self::$mainMenu .= '
                    <li>
                        <a >
                            <label for="categoryId' . $this->action . $val['Category_id'] . '" class="company-name">' . $val['title'] .

                    '</label>
                        </a>';
                $this->convertTreetoLiUlMember(self::$mainList[$val['Category_id']]);
                self::$mainMenu .= '</li>';
            } else {
                self::$mainMenu .= '
                    <li>
                        <a >
                            <label for="categoryId' . $this->action . $val['Category_id'] . '" class="company-name">' . $val['title'] .
                    '<input type="checkbox" name="category_id[]" id="categoryId' . $this->action . $val['Category_id'] . '" value="' . $val['Category_id'] . '">
                            </label>
                        </a>';
            }
            //$mainMenu .= "</ul>\n";
    ...
    public function getCategory($categoryArray)
    {
