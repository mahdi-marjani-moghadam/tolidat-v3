...
class advertiseModelDb
{
    ...
    static function getAdvertiseByCategoryId($id)
    {


        $conn = dbConn::getConnection();
        $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM advertise WHERE";
        
        /*  if (isset($fields['condition']['city_id'])) {
            $sql .= ' city_id = ' . $fields['condition']['city_id'];
        }*/

        if (isset($id)) {
            $sql .= ' (';
            
            foreach ($id['export']['list'] as $key => $value) {
                $sql .= "category_id like '" . $value . "' or 
                         category_id like '" . $value . ",%' or  
                         category_id like '%," . $value . ",%' or 
                         category_id like '%," . $value . "' or ";
            }
            $sql = substr($sql, 0, -3);
            $sql .= ") and type = 'directoryAdvertise' order by sort asc";
        }
