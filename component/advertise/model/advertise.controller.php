...
class advertiseController
{
    ...
    public function getAdvertiseByType($type)
    {
        $directoryAdvertise = advertise::getBy_type($type)->getList();

        return $directoryAdvertise['export']['list'];
    }

    public function getAdvertiseByCategoryId($fields)
    {
        if (!is_array($fields)) {
            $fields = handleData($fields);
            $fields = explode(',', $fields['condition']['category_id']);
        }

        include_once(dirname(__FILE__) . "/advertise.model.db.php");
        $result = advertiseModelDb::getAdvertiseByCategoryId($fields);
        $this->list = $result['export']['list'];

        return $result;
    }

    public function randomAdverb($length = 10)
    {
        $result = advertise::getAll()->getList();
        $count = $result['export']['recordsCount'] - 1;
        ...
        while (count($randomData) < $length) {
            $num = rand(0, $count);
            if (array_key_exists($num, $randomData)) {
                continue;
            } else {
                $randomData[$num] = $result[$num];
            }
        }
