...
class Product extends looeic
{
    protected $TABLE_NAME = 'c_product';

    public function getRelatedProducts($product_id, $keywords)
    {
        $products = Product::getAll()->where('Product_id', '<>', $product_id);
        if (!empty($keywords)) {
            $i = 0;
            $count = count($keywords);
            foreach ($keywords as $keyword) {
                if ($i == 0) {
                    $products->whereOpen('meta_keyword', 'like', '%' . $keyword . '%');
                } else if ($i < $count - 1) {
                    $products->orWhere('meta_keyword', 'like', '%' . $keyword . '%');
                } else {
                    $products->orWhereClose('meta_keyword', 'like', '%' . $keyword . '%');
                }
                $i++;
            }
        }
